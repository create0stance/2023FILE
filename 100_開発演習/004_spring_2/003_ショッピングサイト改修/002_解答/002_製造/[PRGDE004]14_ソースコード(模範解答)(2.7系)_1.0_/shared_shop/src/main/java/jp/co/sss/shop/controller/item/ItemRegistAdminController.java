...
public class ItemRegistAdminController {
	...
	public String registCheck(@Valid @ModelAttribute ItemForm form, BindingResult result) {
		...
		if (form.getImageFile().getSize() > 0) {
			...
			// 追加
			String imagePath = new File("").getAbsolutePath() + msg.getMessage("imageUploadPath", null, Locale.JAPAN);

			// 修正
			// File uploadPath = new File(Constant.FILE_UPLOAD_PATH, imageName);
			File uploadPath = new File(imagePath, imageName);
	...
	public String registComplete(@ModelAttribute ItemForm form) throws SerialException, SQLException {
