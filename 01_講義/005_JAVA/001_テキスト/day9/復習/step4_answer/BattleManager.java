...
public class BattleManager {
	 ...
	 * バトル処理
	 * 
	 * @param atackerName 攻撃者の名前
	 * @param atackPoint ベース攻撃力
	 * @param defenderName 防御者の名前
	 * @param hp 防御者の体力
	 * @return 防御者の残HP
	 * @throws IOException
	 */
	private static int battle(Character atacker,Character defender) throws IOException {
		...
		// 結果をアナウンス
		System.out.println(Setting.battleInfomator + " < "+ atacker.getName() + "の攻撃だワン");
		System.out.println(Setting.bumpLine);
		// 会心の一撃の場合、出力内容を変更する
		int voiceLoopCount =  atackPowerRate == 10 ? 3 : 1;
		String atackText =  atackPowerRate == 10 ? "会心の一撃が炸裂！！\n（＊｀Д´）＝Э）TДT●）" : (int)(atackPowerRate*100) + "%の力で攻撃した！";

		for (int i = 0; i < voiceLoopCount; i++) {
			atacker.outputAtackVoice();
		}
	...
	private static void battleStartInfomation(Character c1,Character c2) throws IOException {
		System.out.println(Setting.bottomLine);
		System.out.print(Setting.battleInfomator + " < ");
